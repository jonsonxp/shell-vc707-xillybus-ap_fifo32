...
if [ $REMOVEIP ]; then
	vivado -nolog -nojournal -mode batch -source scripts/vivado_remove_ip.tcl ./hcode_shell.xpr
	echo "IPs in shell are removed."
	exit 1
fi

if [ $ADDIP ]; then
	vivado -nolog -nojournal -mode batch -source scripts/vivado_add_ip.tcl ./hcode_shell.xpr
